...
CREATE TABLE account.account_balance_history (
    ...
    account_id UUID NOT NULL REFERENCES account.trading_accounts(id),
...
CREATE TABLE strategy.strategy_signals (
    id UUID NOT NULL DEFAULT uuid_generate_v4(),
    strategy_id UUID NOT NULL REFERENCES strategy.trading_strategies(id),
    ...
    PRIMARY KEY (strategy_id, generated_at, id)
);
...
COMMENT ON COLUMN strategy.strategy_signals.id IS '신호 고유 식별자';
COMMENT ON COLUMN strategy.strategy_signals.strategy_id IS '전략 ID (외래키)';
COMMENT ON COLUMN strategy.strategy_signals.stock_code IS '종목 코드';
...
COMMENT ON COLUMN strategy.strategy_signals.generated_at IS '신호 생성 일시 (시계열 파티션 키)';
...
CREATE TABLE risk.risk_violations (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    risk_profile_id UUID NOT NULL REFERENCES risk.risk_profiles(id),
    account_id UUID NOT NULL REFERENCES account.trading_accounts(id),
...
CREATE TABLE market.market_data (
    ...
    PRIMARY KEY (stock_code, data_timestamp, id)
);
...
COMMENT ON COLUMN market.market_data.id IS '데이터 고유 식별자';
COMMENT ON COLUMN market.market_data.stock_code IS '종목 코드';
COMMENT ON COLUMN market.market_data.data_timestamp IS '데이터 시각 (시계열 파티션 키)';
...
BEGIN
    ...
    RAISE NOTICE '================================================================================';
    RAISE NOTICE 'Created schemas: account, trading, strategy, risk, market, monitoring, audit';
    ...
    RAISE NOTICE '================================================================================';
    RAISE NOTICE 'Total Tables Created: 28';
    RAISE NOTICE 'Total Hypertables: 8';
    RAISE NOTICE 'Total Indexes: 50+';
    RAISE NOTICE 'Total ENUM Types: 18';
    RAISE NOTICE '================================================================================';
END $$;
